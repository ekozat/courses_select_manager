...
def parse_courses(input_file_path, output_csv_file):
  try:
    ...
    for blob in course_blobs:
      ...
      if matches:
        ...
        if course_code and course_name and not prerequisites and not restrictions:
          ...
          courses.append(Course(course_code, course_name, prerequisites[0][0].strip(), restriction_data))
    ...
    } for course in non_dupe_courses]

    # Write the data to a CSV file
    with open(output_csv_file, mode='w', newline='', encoding='utf-8') as file:
      fieldnames = [
          "course code", "course name", "prerequisites", "restrictions"
      ]
